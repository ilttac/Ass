...
struct ResultByBoneMatrix
{
	uint ResultByBone[MAX_MODEL_TRANSFORMS]; // 64 * 250 = 160000 byte  // maxStructSize ?? 2048, 4096
	float padding[2];
};

StructuredBuffer<ClipFrameBoneMatrix> Input;
RWStructuredBuffer<ResultMatrix> Output;
RWStructuredBuffer<ResultByBoneMatrix> Output2;
...
cbuffer CB_AnimBone
{
	uint AnimBoneIndex[MAX_MODEL_TRANSFORMS];
}
		...
		uint GroupThreadID : SV_GroupThreadID)
{
	...
	Output2[index].ResultByBone[index] = 0;
	//Output2[index].ResultByBone[GroupThreadID] = result;
}
