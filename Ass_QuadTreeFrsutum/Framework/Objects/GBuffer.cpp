...
void GBuffer::Lighting()
{
	...
	//PointLight - Debug
	{
		...
		UINT count = Context::Get()->PointLights(pointLights);
		for (UINT i = 0; i < count; i++)
		{
			Matrix S, T;
			float s = pointLights[i].Range;
			Vector3 t = pointLights[i].Position;

			D3DXMatrixScaling(&S, s, s, s);
			D3DXMatrixTranslation(&T, t.x, t.y, t.z);

			pointLightDesc.Projection = S * T * Context::Get()->View() * Context::Get()->Projection();
			pointLightDesc.PointLight = pointLights[i];
