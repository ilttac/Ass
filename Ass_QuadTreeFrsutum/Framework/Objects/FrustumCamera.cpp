...
FrustumCamera::FrustumCamera(Transform * transform, Transform * init)
	: transform(transform), init(init)
{
	lines[0] = Vector3(-0.5f, -0.5f, -0.5f);
	lines[1] = Vector3(-1.5f, +0.5f, +0.5f);
	lines[2] = Vector3(+0.5f, +0.5f, +0.5f);
	lines[3] = Vector3(+0.5f, -1.5f, +0.5f);
	lines[4] = Vector3(-1.5f, -1.5f, +0.5f);
}
...
bool FrustumCamera::IsIntersect(FrustumCamera * other)
{
	return Collision(this->obb, other->obb);
}

void FrustumCamera::Update()
{
	SetObb();
}

void FrustumCamera::Render(Color color)
{
	Vector3 dest[8];
...
bool FrustumCamera::SeperatingPlane(Vector3 & position, Vector3 & direction, Obb & box1, Obb & box2)
{
...
bool FrustumCamera::Collision(Obb & box1, Obb & box2)
{
...
Vector3 FrustumCamera::Cross(Vector3 & vec1, Vector3 & vec2)
{
