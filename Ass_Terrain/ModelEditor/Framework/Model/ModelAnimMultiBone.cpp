#include "Framework.h"
#include "ModelAnimMultiBone.h"
#include "ModelMesh.h"

ModelAnimMultiBone::ModelAnimMultiBone(Shader* shader)
	: shader(shader)
{
	model = new Model();
	...
	computeShader = new Shader(L"35_ColliderMulti.fx");
	...
	sComputeBoneBuffer = computeShader->AsConstantBuffer("CB_AttachAllBones");
}

ModelAnimMultiBone::~ModelAnimMultiBone()
{
...
void ModelAnimMultiBone::Update()
{
	...
	if (computeBuffer != NULL)
	{
		...
		{
			boneMatrix[i] = csOutput[i].Result;
		}
	}
...
void ModelAnimMultiBone::Render()
{
	if (texture == NULL)
	{
		CreateTexture();
...
void ModelAnimMultiBone::ReadMaterial(wstring file)
{
	model->ReadMaterial(file);
}

void ModelAnimMultiBone::ReadMesh(wstring file)
{
	model->ReadMesh(file);
...
void ModelAnimMultiBone::ReadClip(wstring file)
{
	model->ReadClip(file);
}

void ModelAnimMultiBone::PlayClip(UINT instance, UINT clip, float speed, float takeTime)
{
...
void ModelAnimMultiBone::Pass(UINT pass)
{
	for (ModelMesh* mesh : model->Meshes())
	{
		mesh->Pass(pass);
...
Transform* ModelAnimMultiBone::AddTransform()
{
...
void ModelAnimMultiBone::UpdateTransforms()
{
	for (UINT i = 0; i < transforms.size(); i++)
...
Matrix ModelAnimMultiBone::GetAttachTransform(UINT index)
{
	if (csOutput == NULL)
	{
		Matrix temp;
...
void ModelAnimMultiBone::CreateTexture()
{
...
void ModelAnimMultiBone::CreateClipTransform(UINT index)
{
...
void ModelAnimMultiBone::CreateComputeDesc()
{
