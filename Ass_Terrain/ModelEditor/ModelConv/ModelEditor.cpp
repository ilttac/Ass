...
void ModelEditor::Mesh()
{
	...
	//Create Mesh
	{
		...
		/*sphere2 = new MeshRender(modelShader, new MeshSphere(0.5f, 20, 20));
		for (UINT i = 0; i < MAX_MODEL_INSTANCE; i++)
		{
			Transform* transform2 = NULL;
			transform2 = sphere2->AddTransform();
			D3DXMatrixIdentity(&transform2->World());
		}
		for (UINT i = 0; i < MAX_MODEL_INSTANCE; i++)
		{
			sphere2->GetTransform(i)->World()._41 = i+2;
		}*/
	}
			...
			for (int i = 0; i < hiarachyName.size(); i++)
			{
				...
				if (ImGui::IsItemClicked())
				{
					...
					if (bCount == true)
					{
						...
						{
							sphere->GetTransform(i)->Scale(0.0f, 0.0f, 0.0f);
				...
				if (node_open)
				{
					for (UINT k = 0; k < modelLists[currentModelID]->GetModel()->BoneCount(); k++)
					{
						//string str = String::ToString(modelLists[currentModelID]->GetModel()->Bones()[i]->Name())
						if (ImGui::TreeNode(String::ToString(modelLists[currentModelID]->GetModel()->Bones()[k]->Name()).c_str()))
						{
							if (ImGui::IsItemClicked())
							{
								editorState = BONE_EDITOR_STATE;
...
void ModelEditor::Weapon()
{
	weapon = new ModelRender(modelShader);
	weapon->ReadMaterial(L"Weapon/Sword");
	weapon->ReadMesh(L"Weapon/Sword");
	Transform* transform = NULL;

	transform = weapon->AddTransform();
