...
void SceneEditor::Pass(UINT meshPass)
{
	for (MeshRender* mesh : meshes)
		mesh->Pass(meshPass);
}

void SceneEditor::UpdateParticleList()
{
	particleList.clear();
...
void SceneEditor::UpdateTextureList()
{
	textureList.clear();
...
void SceneEditor::OnGUI_Settings()
{
...
void SceneEditor::OnGuI_Write()
{
	...
	if (ImGui::Button("SaveParticle"))
	{
		...
		(
			...
			bind(&SceneEditor::WrtieFile, this, placeholders::_1),
			desc.Handle
		);
	}
}

void SceneEditor::WrtieFile(wstring file)
{
