...
cbuffer CB_TerrainLod
{
	...
	float WorldCellSpace;//지면 한칸의 킈기
	float TerrainHeightRatio;//높이를 너무크거나 저무작게 나오지않게 하는 변수.

	float2 TextureScale;
	float CB_Trreain_Padding[2];
...
struct VertexInput_TerrainLod
{
	...
	float2 Uv : Uv;
	float2 BoundY : BoundY;//그셀이 가질수있는 최소높이
};
...
float TessellationFactor(float3 position)
{
	...
	float s = saturate((d - MaxDistance) / (MinDistance-MaxDistance)); //거리가 멀수록 작아짐
...
ConstantHullOutput_TerrainLod CHS_TerrainLod(InputPatch<VertexOutput_TerrainLod, 4> input)
{
	...
	float3 c = (input[0].Position + input[1].Position + input[2].Position + input[3].Position).xyz *0.25;
...
struct DomainOutput_TerrainLod
{
	...
	float2 Uv : Uv0;//전체 uv
	float2 Tileduv : Uv1;//한개의 셀에대한 uv
};
...
{
	float2 left = input.Uv * float2(-TexelCellSpaceU, 0.0f); //쪼개진 uv
	float2 right = input.Uv * float2(+TexelCellSpaceU, 0.0f); //쪼개진 uv
	float2 top = input.Uv * float2(0.0f, -TexelCellSpaceU); //쪼개진 uv
	float2 bottom = input.Uv * float2(0.0f, +TexelCellSpaceU); //쪼개진 uv
	...
	float3 biTangent = normalize(float3(0.0f,bottomY-topY , WorldCellSpace * 2.0f));//정규직교화로 하는게 정석
