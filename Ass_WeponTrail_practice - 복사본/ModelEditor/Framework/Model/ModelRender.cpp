	...
	: shader(shader)
{
	...
	boneBuffer = new ConstantBuffer(&changedBoneDesc, sizeof(ChangedBoneDesc));
	sBoneBuffer = shader->AsConstantBuffer("CB_ChangedBone");
}
...
void ModelRender::UpdateTransform(UINT instanceId, UINT boneIndex, Transform & transform)
{
	...
	//model->BindBone();
	//model->BindMesh();
	changedBoneDesc.ChangedBoneIndex = boneIndex;
	changedBoneDesc.ChagnedTransfrom = transform.World();
}
