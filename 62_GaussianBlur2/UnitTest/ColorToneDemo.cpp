...
void ColorToneDemo::Initialize()
{
	...
	postEffect = new PostEffect(L"59_ColorTone.fx");
...
void ColorToneDemo::Update()
{
	...
	{
		"None" , "Inverse","GrayScale","GrayScale2","Green","Sepia",
		"Saturation" , "Sharpness","Vignette","Interlace","LensDistorition", "Wiggle"
	};
	ImGui::LabelText("Mode Name", "%s", effectName[Pass]);
	...
	//Vignette
	{
		...
		ImGui::InputFloat("VignettePower", &Power, 0.05f);
		postEffect->GetShader()->AsScalar("Power")->SetFloat(Power);
	
		static float ScaleX = 2.0f;
		ImGui::InputFloat("SclaeX", &ScaleX, 0.1f);
	
		static float ScaleY = 2.0f;
		ImGui::InputFloat("SclaeY", &ScaleY, 0.1f);
	...
	//Interlace
	{
		...
		ImGui::InputFloat("InterlaceStr", &Strength, 0.01f);
		postEffect->GetShader()->AsScalar("Strength")->SetFloat(Strength);
	
		static int InterlaceValue = 2;
		ImGui::InputInt("InterlaceValue", &InterlaceValue);
		postEffect->GetShader()->AsScalar("InterlaceValue")->SetInt(InterlaceValue);
	}
	...
	//Wiggle
	{
		...
		static float AmountX = 0.01;
		ImGui::InputFloat("AmountX", &AmountX, 0.1f);
		
		static float AmountY = 0.01;
		ImGui::InputFloat("AmountY", &AmountY, 0.1f);
