...
wstring Path::GetLastDirectoryName(wstring path)
{
	wstring directoryName = GetDirectoryName(path);
	directoryName.pop_back();
	size_t index = directoryName.find_last_of('/');
	
	return directoryName.substr(index + 1, directoryName.length());
}
